...
buildJsInLocales = ->
    path = require 'path'
    for file in fs.readdirSync './build/client/app/locales/'
        filename = './build/client/app/locales/' + file
        template = fs.readFileSync filename, 'utf8'
        output = "module.exports = "
        output += template
        output += ';'
        name = file.replace '.json', '.js'
        fs.writeFileSync "./build/client/app/locales/#{name}", output
...
task 'build', 'Build CoffeeScript to Javascript', ->
    ...
    command = "coffee -cb --output build/server server && " + \
              ...
              "cp -rf client/app/locales/*.json build/client/app/locales/ && " + \
